<script>
	...
	import { DaerahService } from '$lib/services/masterData.js';
	...
	async function handleDelete(item) {
		if (!confirm(`Apakah Anda yakin ingin menghapus ${item.nama_daerah}?`)) {
			return;
		}
	...
	async function handleSave() {
		try {
			...
			console.error('Error saving daerah:', error);
		}
	}
</script>
...
<div class="content-area">
	<div class="content-header">
		<div class="header-info">
			<h1>Daerah</h1>
			<p class="data-count">{daerahList.length} data</p>
		</div>
		...
		{:else}
			<div class="table-container">
				<table>
					...
					<tbody>
						{#each daerahList as item, index}
							<tr>
								<td>{index + 1}</td>
								<td class="name-cell">{item.nama_daerah}</td>
