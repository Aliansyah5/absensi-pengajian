<script>
	...
	import { HadistService } from '$lib/services/masterData.js';
	...
	async function handleDelete(item) {
		if (!confirm(`Apakah Anda yakin ingin menghapus ${item.nama_hadist}?`)) {
			return;
		}
		...
		try {
			...
			console.error('Error deleting hadist:', error);
		}
	}
	...
	async function handleSave() {
		try {
			...
			console.error('Error saving hadist:', error);
		}
	}
</script>
...
<div class="content-area">
	<div class="content-header">
		<div class="header-info">
			<h1>Hadist</h1>
			<p class="data-count">{hadistList.length} data</p>
		</div>
		...
		{:else}
			<div class="table-container">
				<table>
					...
					<tbody>
						{#each hadistList as item, index}
							<tr>
								<td>{index + 1}</td>
								<td class="name-cell">{item.nama_hadist}</td>
