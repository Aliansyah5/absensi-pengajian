<script>
	...
	import { DesaService } from '$lib/services/masterData.js';
	...
	async function handleDelete(item) {
		if (!confirm(`Apakah Anda yakin ingin menghapus ${item.nama_desa}?`)) {
			return;
		}
...
<div class="content-area">
	<div class="content-header">
		<div class="header-info">
			<h1>Desa</h1>
			<p class="data-count">{desaList.length} data</p>
		</div>
		...
		{:else}
			<div class="table-container">
				<table>
					...
					<tbody>
						{#each desaList as item, index}
							<tr>
								<td>{index + 1}</td>
								<td class="name-cell">{item.nama_desa}</td>
