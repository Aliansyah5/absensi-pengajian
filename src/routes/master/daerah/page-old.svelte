<script>
	...
	import { MasterDataService, DaerahService } from '$lib/services/masterData.js';
	import AppHeader from '$lib/components/AppHeader.svelte';
	import BottomNav from '$lib/components/BottomNav.svelte';
	import {
		Plus, Edit, Trash2, Eye, Search, RefreshCw, Save, X, ArrowLeft, MapPin
	} from 'lucide-svelte';
	...
	async function loadData() {
		isDataLoading = true;
		try {
			dataList = await DaerahService.getAllDaerah();
			console.log('Daerah data loaded:', dataList);
		} catch (error) {
	...
	function validateForm() {
		...
		if (!formData.nama?.trim()) {
			formErrors.nama = 'Nama daerah wajib diisi';
		}
	...
	async function handleSubmit() {
		...
		try {
			if (modalMode === 'create') {
				...
				showMessage('success', 'Data daerah berhasil ditambahkan');
			} else {
				await DaerahService.updateDaerah(formData.id, formData);
				showMessage('success', 'Data daerah berhasil diperbarui');
			}
	...
	async function handleDelete(item) {
		...
		try {
			...
			showMessage('success', 'Data daerah berhasil dihapus');
			await loadData();
		} catch (error) {
...
<svelte:head>
	<title>Master Daerah - Absensi Pengajian</title>
</svelte:head>
		...
		{:else if isSuperAdmin}
			...
			<div class="flex items-center justify-between mb-6">
				<div class="flex items-center gap-3">
					...
					<div class="flex items-center gap-3">
						<div class="icon-badge bg-gradient-to-br from-green-500 to-green-600">
							<MapPin size={24} color="white" />
						</div>
						<div>
							<h1 class="text-2xl font-bold text-gray-800">Master Daerah</h1>
							<p class="text-sm text-gray-600">Kelola data daerah</p>
						</div>
					</div>
				</div>
			...
			{:else if filteredData.length === 0}
				<div class="empty-state">
					...
					<p class="text-gray-600 text-lg font-medium">
						{searchQuery ? 'Tidak ada data yang cocok' : 'Belum ada data daerah'}
					</p>
					<p class="text-gray-500 text-sm">
						{searchQuery ? 'Coba kata kunci lain' : 'Klik tombol "Tambah Data" untuk menambahkan daerah'}
					</p>
				</div>
			{:else}
			...
			<div class="modal-content" on:click|stopPropagation>
				<div class="modal-header">
					<h2 class="modal-title">
						{modalMode === 'create' ? 'Tambah' : modalMode === 'edit' ? 'Edit' : 'Detail'} Daerah
					</h2>
...
<style>
	/* Import same styles as pengajian */
	.btn-back {
		display: flex;
		align-items: center;
